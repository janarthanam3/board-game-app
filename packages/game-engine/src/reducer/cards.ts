  ...
  if (rule.money) {
    for (const transfer of moneyPlan(state, playerId, rule.money)) {
      if (transfer.from === "bank") {
        ...
        // OQ-22 item 2: a card's "You pay bank" is a fine, so a pot board collects it like a tax.
        const payTo = transfer.to === "bank" ? "fine" : "creditor";
        const result = charge(ctx, transfer.from, transfer.to, transfer.amount, `card: ${rule.name}`, payTo);
